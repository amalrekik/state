...
componentDidUpdate(){
 if ( this.state.timer>=20)
  { clearInterval(this.conteur); console.log("bye")}
...
<button onClick={(event) => { handleShowPhoto(); voirProfil();}}>{this.state.appar?("cacher"):("montrer")} profil</button><br/>
    ...
    </div>):(<div><h3>pour voir monprofil clickez sur "montrer profil"</h3><p>mon profil est visité depuis {this.state.timer} s</p></div>)}
    </div>
  );
}
}
